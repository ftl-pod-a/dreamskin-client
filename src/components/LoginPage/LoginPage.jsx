...
const Login = () => {
  ...
  const getRecommendedProducts = async () => {
    try {
        ...
        const response = await axios.get(`https://dreamskin-server-tzka.onrender.com/http://localhost:3000/routine/${userId}`);
        localStorage.setItem('products', JSON.stringify(response.data.products));
        console.log("Response", response.data.products);
        const response2 = await axios.get(`https://dreamskin-server-tzka.onrender.com/http://localhost:3000/users/${userId}`, {user_id: userId});
