...
const Product = ({ product_id, name, brand, price, liked, imageUrl, ingredients, description}) => {
    ...
    const getLikedProducts = async (params = {}) => {
        try {
            const response = await axios.get(`https://dreamskin-server-tzka.onrender.com/http://localhost:3000/users/${userId}`, {user_id: userId});
    ...
    const handleLike = async () => {
        try {
            const response = await axios.post(`https://dreamskin-server-tzka.onrender.com/http://localhost:3000/products/${product_id}/like`, {
                userId: userId,
            });
            const response1 = await axios.get(`https://dreamskin-server-tzka.onrender.com/http://localhost:3000/users/${userId}`, {user_id: userId});
